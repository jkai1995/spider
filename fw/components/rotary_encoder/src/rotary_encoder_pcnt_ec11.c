...
static esp_err_t ec11_sample_encoder(rotary_encoder_t *encoder)
{
    ec11_t *ec11 = __containerof(encoder, ec11_t, parent);
    ...
    ec11->count = val + ec11->accumu_count;
    ec11->rspeedRPS = ec11->count * ec11->config.cnt2rps;
...
static int ec11_get_counter_value(rotary_encoder_t *encoder,float *pRPS)
{
    ec11_t *ec11 = __containerof(encoder, ec11_t, parent);

    int16_t count = ec11->count;
    if (pRPS != NULL)
    {
        *pRPS = ec11->rspeedRPS;
    }
...
static esp_err_t ec11_clear_counter_value(rotary_encoder_t *encoder)
{
    ec11_t *ec11 = __containerof(encoder, ec11_t, parent);
    pcnt_counter_clear(ec11->pcnt_unit);
    return ESP_OK;
...
esp_err_t rotary_encoder_new_ec11(const rotary_encoder_config_t *config, rotary_encoder_t **ret_encoder)
{
    ...
    ec11->parent.clear_counter_value = ec11_clear_counter_value;
    ec11->parent.sample_encoder    =  ec11_sample_encoder;

    memcpy(&(ec11->config), config, sizeof(rotary_encoder_config_t));

    if (config->samplingPeriod_ms > 0 && config->pulseCntPR > 0)
    {
        ec11->config.cnt2rps = 1.0*1000/config->samplingPeriod_ms/config->pulseCntPR;
    }
    else
    {
        ec11->config.cnt2rps = 0;
    }
