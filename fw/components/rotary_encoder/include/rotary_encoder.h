...
typedef struct {
    ...
    float cnt2rps;//(cnt * cnt2rpm) = RevolutionsPerSecond
    int pulseCntPR;//pulseCnt per revolutions
    int samplingPeriod_ms;
...
struct rotary_encoder_t {
     ...
     */
     int (*sample_encoder)(rotary_encoder_t *encoder);
    int (*get_counter_value)(rotary_encoder_t *encoder,float *pRPS);
    esp_err_t (*clear_counter_value)(rotary_encoder_t *encoder);
};
